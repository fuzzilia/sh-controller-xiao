...
static void PushKeys(std::queue<KeyboardValue> &keys, KeyboardValue key1,
                     KeyboardValue key2) {
...
int SHController::ProcessRotationState::ProcessWithRelativeRotate(
    float rotate) {
    const auto current_rotate = m_last_rotate + rotate;
    float rotate_offset = 0;
    if ((m_first_direction == 0 && current_rotate < 0) ||
        m_first_direction == -1) {
        rotate_offset = 0.99999999;
    }

    const float rotate_diff_from_split_point =
        current_rotate + rotate_offset - (float)m_count;
    int count = (int)floorf(current_rotate + rotate_offset);
    if ((m_last_direction == -1 && rotate_diff_from_split_point > 0 &&
         rotate_diff_from_split_point < 1.25f) ||
        (m_last_direction == 1 && rotate_diff_from_split_point < 0 &&
         rotate_diff_from_split_point > -1.25f)) {
        count = m_count;
    }
...
void ProcessCountDiff(std::queue<KeyboardValue> &keys, int count_diff,
                      const PositiveAndNegative<KeyboardValue> &config) {
    if (config.positive) {
...
NullableThreeDimension SHController::ProcessRotationState::LockedAxis(
    const ThreeDimensionValue<ProcessRotationState> &states) {
...
void SHController::tick(std::queue<KeyboardValue> &keys) {
    ...
    for (uint8_t button_index = 0;
         button_index < m_config->CombinationButtonNumbers().size();
         button_index++) {
        if (m_read_button_is_on(
                m_config->CombinationButtonNumbers()[button_index])) {
    ...
    for (uint8_t button_index = 0;
         button_index < m_config->StandardButtonNumbers().size();
         button_index++) {
        if (m_read_button_is_on(
                m_config->StandardButtonNumbers()[button_index])) {
    ...
    if (combination_state != m_last_combination_state ||
        button_number != m_last_button_number) {
    ...
    if (buttonBlockType != ButtonBlockType::Empty) {
        ...
        case ButtonBlockType::Rotation:
            if (button_state_changed) {
                m_state.rotate_motion = {.locked_axis =
                                             NullableThreeDimension::Null,
                                         .rotation_state = {
            ...
            ProcessButtonRotation(
                keys, m_config->RotateButtonConfigAt(button.ReferenceIndex()));
            return;
        }
    }

    uint16_t stick_config = 0;
    for (uint8_t stick_index = 0; stick_index < m_config->StickCount();
         stick_index++) {
        const auto &stick =
            m_config->GetStickBlock(combination_state, stick_index);
        ...
        const float normalized_angle =
            fmodf((float)1.25f - angle * (float)M_1_PI * 0.5 +
                      m_config->StickRotateOffset(stick_index),
                  1.0f);

        switch (block_type) {
        case StickBlockType::FourButton: {
            const auto &config =
                m_config->FourButtonStickConfigAt(stick.ReferenceIndex());
        ...
        case StickBlockType::EightButton: {
            const auto &config =
                m_config->EightButtonStickConfigAt(stick.ReferenceIndex());
        ...
        case StickBlockType::Rotate: {
            const auto &config =
                m_config->RotateStickConfigAt(stick.ReferenceIndex());
            if (button_state_changed) {
                m_state.stick_rotation = {ProcessRotationState(),
            ...
            float normalized_angle_diff =
                normalized_angle - m_state.stick_rotation.last_angle;
            ...
            int count_diff =
                m_state.stick_rotation.rotation.ProcessWithRelativeRotate(
...
void SHController::ProcessButtonRotation(
    std::queue<KeyboardValue> &keys,
    const SHConfig::RotateButtonConfig &config) {
    ...
    bool xIsActive =
        !axis_is_locked || locked_axis == NullableThreeDimension::X;
    bool yIsActive =
        !axis_is_locked || locked_axis == NullableThreeDimension::Y;
    bool zIsActive =
        !axis_is_locked || locked_axis == NullableThreeDimension::Z;
    for (const auto &sensor_value : sensor_values) {
        if (xIsActive) {
            adding_rotation.x += sensor_value.gyro.x *
                                 config.rotate_split_size.x *
                                 sensor_value.time_span_s;
        }
        if (yIsActive) {
            adding_rotation.y += sensor_value.gyro.y *
                                 config.rotate_split_size.y *
                                 sensor_value.time_span_s;
        }
        if (zIsActive) {
            adding_rotation.z += sensor_value.gyro.z *
                                 config.rotate_split_size.z *
                                 sensor_value.time_span_s;
        }
    }
    int x_diff =
        xIsActive
            ? m_state.rotate_motion.rotation_state.x.ProcessWithRelativeRotate(
                  adding_rotation.x)
            : 0;
    int y_diff =
        yIsActive
            ? m_state.rotate_motion.rotation_state.y.ProcessWithRelativeRotate(
                  adding_rotation.y)
            : 0;
    int z_diff =
        zIsActive
            ? m_state.rotate_motion.rotation_state.z.ProcessWithRelativeRotate(
                  adding_rotation.z)
            : 0;

    NullableThreeDimension next_locked_axis = locked_axis;
    if (next_locked_axis == NullableThreeDimension::Null && config.locks_axis) {
        next_locked_axis = ProcessRotationState::LockedAxis(
            m_state.rotate_motion.rotation_state);
...
void SHController::ProcessButtonGesture(
    std::queue<KeyboardValue> &keys,
    const SHConfig::GestureButtonConfig &config) {
    // TODO 実装
}

const SHConfig &SHController::config() const { return *m_config; }
